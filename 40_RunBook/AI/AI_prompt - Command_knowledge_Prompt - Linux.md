...
- コマンド名を入力したらこのテンプレートの全項目を最大密度で出力してください
- 準備ができたらコマンド名を入力してくださいとだけ返してください
- １回の回答による精度低下を防ぐため３段階に分けて出力してください
	- １回目
		- 概要
		- 類似コマンドと差異
		- よく連携されるコマンド
	...
	- ２回目
		- よく使われるオプション組み合わせと業務シナリオ
			- インフラ・ブルー・レッドの３つの視点は該当する場合のみ記述
			- 特定の立場で極めて重要な手法がある場合はその項目にリソースを集中させる
	- ３回目
		- エラーメッセージとトラブルシューティング
		- 環境変数と設定ファイル
		- セキュリティに関する考慮事項
		- 注意点・補足
- １つのチャットでは最大５コマンドまでの出力とする
	- ５コマンドを出力したなら新しいチャットに切り替える旨のメッセージを出力
