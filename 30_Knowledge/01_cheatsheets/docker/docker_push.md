...
tags:
  ...
  - 'registry'
  - 'cheatsheet'
title: 'docker push - Dockerイメージをレジストリにアップロードする'
summary: 'ローカルに存在するDockerイメージを、Docker Hubやプライベートレジストリなどのリモートレジストリにアップロード（プッシュ）します。'
related:
  - 'docker_pull'
  - 'docker_tag'
  - 'docker_login'
...
# `docker push` - Dockerイメージをレジストリにアップロードする
