...
tags:
  ...
  - 'cleanup'
  - 'cheatsheet'
title: 'docker system - Dockerシステム全体を管理する'
summary: 'Dockerデーモンの管理、ディスク使用量の確認、未使用リソースの一括削除など、Dockerシステム全体に関わる操作を行うコマンドスイートです。'
related:
  - 'docker_info'
  - 'docker_version'
  - 'docker_system_df'
...
# `docker system` - Dockerシステム全体を管理する
...
| コマンド | 概要 | 詳細 |
| :--- | :--- | :--- |
| ⭐ **`docker system df`** | Dockerが使用する**ディスク容量**の内訳を表示する | [./docker_system_df.md](./docker_system_df.md) |
| ⭐ **`docker system prune`** | 未使用のコンテナ、ネットワーク、イメージなどの**リソースを一括削除**する | [./docker_system_prune.md](./docker_system_prune.md) |
| **`docker system info`** | Dockerシステムの**詳細情報**を表示する (`docker info` と同義) | [./docker_info.md](./docker_info.md) |
