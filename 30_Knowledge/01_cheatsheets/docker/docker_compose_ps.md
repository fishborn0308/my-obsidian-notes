...
tags:
  ...
  - 'docker'
  - 'cheatsheet'
title: 'docker compose ps - Composeアプリケーションのコンテナ状態を表示する'
summary: 'docker-compose.ymlで定義されたアプリケーションを構成するコンテナの一覧と、その状態(State)、ポートマッピング(Ports)などを表示します。'
related:
  - 'docker_ps'
  - 'docker_compose_up'
  - 'docker_compose_logs'
...
# `docker compose ps` - Composeアプリケーションのコンテナ状態を表示する
