...
tags:
  ...
  - troubleshooting
  - cheatsheet
title: docker inspect - Dockerオブジェクトの詳細情報を表示する
summary: コンテナ、イメージ、ボリューム、ネットワークなど、Dockerオブジェクトに関する低レベルで詳細な情報をJSON形式で出力します。
related:
  - docker_ps
  - docker_images
  - docker_info
...
# `docker inspect` - Dockerオブジェクトの詳細情報を表示する
