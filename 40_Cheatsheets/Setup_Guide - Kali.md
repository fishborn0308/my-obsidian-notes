...
# 2. fzfでソースを選択（WorkbenchとSecListsの両方を検索）

# findに $WORDLISTS と $SECLISTS の両方を与え、優先度の高い順に表示

local selected_list=$(find "$WORDLISTS" "$SECLISTS" -maxdepth 4 -type f 2>/dev/null | fzf \

--prompt="Select Wordlist > " \

--preview="echo '[ PATH: {} ]'; echo '[ SIZE: \$(wc -l < {}) lines ]'; echo '------------------'; head -n 15 {}")
...
# 辞書名を環境変数にエクスポート（ファイル名部分のみ抽出）

export ACTIVE_DICT=$(basename "$selected_list")

# 実行ログの表示

echo -e "\033[1;32m[+] Linked:\033[0m $selected_list -> $link_name"

echo -e "\033[1;34m[*] Exported:\033[0m ACTIVE_DICT=$ACTIVE_DICT"
...
tmux set-environment -g @logging-path "$HOME/Vault/Target/$ip/log"
...
set -g status-right "#[fg=black,bg=red,bold] TARGET: #$TARGET_IP (#$TARGET_NAME) #[default] #[fg=yellow]%H:%M:%S "
